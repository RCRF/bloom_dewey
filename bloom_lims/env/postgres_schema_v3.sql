...
*/
-- NOT handling is_deleted here...
-- function to query for an euid across all tables which have an euid column
CREATE OR REPLACE FUNCTION query_for_euid(search_value TEXT)
RETURNS TABLE(table_name TEXT, euid TEXT, uuid UUID, super_type TEXT, polymorphic_discriminator TEXT) AS $$
DECLARE
    table_record RECORD;
BEGIN
    FOR table_record IN 
        ...
        WHERE c.table_schema = 'public'  -- Replace 'public' with your schema name if different
        AND c.column_name = 'euid'
        AND EXISTS (
            SELECT 1
    ...
    LOOP
        RETURN QUERY EXECUTE format(
            'SELECT %L, euid, uuid, super_type, polymorphic_discriminator FROM %I WHERE euid = %L',
            table_record.table_name, table_record.table_name, search_value
        );
    END LOOP;
END;
$$ LANGUAGE plpgsql;
...
-- function to query for uuid across all tables which have a uuid column
CREATE OR REPLACE FUNCTION query_for_uuid(search_value_uuid UUID)
RETURNS TABLE(table_name TEXT, euid TEXT, uuid UUID, super_type TEXT, polymorphic_discriminator TEXT) AS $$
DECLARE
    table_record RECORD;
BEGIN
    FOR table_record IN 
        ...
        AND EXISTS (
            ...
            WHERE c2.table_schema = c.table_schema
            AND c2.table_name = c.table_name
            AND c2.column_name = 'euid'
        )
    LOOP
        RETURN QUERY EXECUTE format(
            'SELECT %L, euid, uuid, super_type, polymorphic_discriminator FROM %I WHERE uuid = %L',
            table_record.table_name, table_record.table_name, search_value_uuid
        );
    END LOOP;
END;
$$ LANGUAGE plpgsql;
